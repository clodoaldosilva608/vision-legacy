...
const boards = [
  ...
  { t: "Mente elevada", color: "from-cyan-400 to-blue-600", items: 14, cat: "Growth", icon: "brain" as IconName },
];
    ...
    <UserLayout>
      ...
      {tab === "Metas do board" && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {["Casa própria em 2028", "Renda passiva R$ 5k/mês", "50 livros em 2026", "Maratona 42km", "30 países visitados", "Publicar primeiro livro"].map((m) => (
            <Card key={m}>
              <h3 className="text-lg font-bold text-slate-800">{m}</h3>
              <div className="mt-3"><span className="text-xs font-bold text-slate-400">Progresso</span><div className="mt-2 h-3 w-full rounded-full bg-slate-100"><div className="h-3 rounded-full bg-gradient-to-r from-accent to-gold" style={{ width: `${20 + Math.floor(Math.random() * 70)}%` }} /></div></div>
