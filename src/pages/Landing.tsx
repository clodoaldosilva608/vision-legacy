...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-dark text-white overflow-x-hidden">
      ...
      {/* ── NAV ── */}
      <header className={`sticky top-0 z-50 border-b transition-all duration-300 ${scrolled?"border-white/8 bg-dark/95 shadow-lg shadow-black/20 backdrop-blur-xl":"border-transparent bg-transparent"}`}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4">
          ...
          <div className="flex items-center gap-3">
            <Link to="/login" className="hidden text-sm font-semibold text-white/70 hover:text-white sm:block transition">Entrar</Link>
            <Link to="/onboarding" className="gold-gradient rounded-xl px-4 py-2 text-sm font-bold text-dark transition hover:brightness-110 shadow-lg shadow-gold/20">
              Começar agora
            </Link>
        ...
        {mobileOpen&&(
          <nav className="flex flex-col gap-1 border-t border-white/5 px-5 py-4 lg:hidden bg-dark/95">
            ...
            <Link to="/login" className="mt-1 rounded-lg px-3 py-2.5 text-sm font-semibold text-white/70">Entrar</Link>
          </nav>
        )}
      </header>
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-16 sm:py-24 lg:grid-cols-2">
          <div className="animate-fade-up">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link to="/onboarding" className="gold-gradient inline-flex items-center gap-2 rounded-xl px-6 py-3.5 text-sm font-bold text-dark shadow-xl shadow-gold/25 transition hover:brightness-110 hover:scale-105">
                Começar minha jornada <Icon.arrowRight className="h-4 w-4"/>
              </Link>
              <Link to="/pricing" className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-6 py-3.5 text-sm font-semibold text-white backdrop-blur transition hover:bg-white/10">
        ...
        <div className="noise relative overflow-hidden rounded-3xl border border-accent/20 bg-gradient-to-br from-secondary via-secondary to-accent/10 p-8 sm:p-12">
          ...
          <div className="grid items-center gap-10 lg:grid-cols-2">
            <div className="animate-slide-left">
              ...
              <Link to="/onboarding" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-accent px-5 py-3 text-sm font-bold text-white transition hover:bg-accent-light">
      ...
      <section className="mx-auto max-w-7xl px-5 pb-20">
        ...
        <div className="mt-12 grid gap-5 lg:grid-cols-4">
          {plans.map((p,i)=>(
            <div key={p.name} className={`animate-fade-up delay-${(i+1)*100} card-hover relative flex flex-col rounded-2xl border-2 p-6 transition
              ...
              <Link to="/onboarding" className={`mt-6 block rounded-xl py-3 text-center text-sm font-bold transition
        ...
        <div className="mt-6 text-center">
          <Link to="/pricing" className="text-sm font-semibold text-accent-light hover:text-accent transition">
        ...
        <div className="noise relative overflow-hidden rounded-3xl border border-accent/30 bg-gradient-to-br from-accent/20 via-secondary to-gold/10 p-8 text-center sm:p-16">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <Link to="/onboarding" className="gold-gradient rounded-xl px-8 py-4 text-sm font-extrabold text-dark shadow-xl shadow-gold/25 transition hover:brightness-110 hover:scale-105">
                Começar gratuitamente →
              </Link>
              <Link to="/login" className="rounded-xl border border-white/20 bg-white/5 px-8 py-4 text-sm font-bold text-white backdrop-blur transition hover:bg-white/10">
                Já tenho conta
              </Link>
            </div>
          </div>
          ...
          {[
            ...
            <div key={t as string}>
              ...
              <ul className="mt-4 space-y-2.5">
                {(items as {l: string; h: string}[]).map(it=><li key={it.l}><a href={it.h} className="text-sm text-white/50 hover:text-white transition">{it.l}</a></li>)}
              </ul>
            </div>
          ))}
        </div>
